...
@section('buttons')
    ...
    <div class="col-sm-5">
        @if ($mode == config('global.show'))
            <a class="btn btn-primary pl-3" id="button-close" href="@yield('form.close')" role="button">Закрыть</a>
        @else
            <button type="submit" class="btn btn-primary" id="button-save">Сохранить</button>
            <a class="btn btn-secondary pl-3" id="button-close" href="@yield('form.close')" role="button">Закрыть</a>
        @endif
    </div>
@endsection
					...
					@case('file')
						<div class="col-sm-5">
							<input type="{{ $field['type'] }}" class="form-control" id="{{ $field['name'] }}"
								name="{{ $field['name'] }}"
								@if (isset($field['length'])) maxlength="{{ $field['length'] }}" @endif
						...
						<div class="col-sm-8">
							<div class="form-check form-switch">
								<input class="form-check-input" type="checkbox" id="{{ $field['name'] }}"
									name="{{ $field['name'] }}" @if (isset($field['value']) && $field['value']) checked @endif
									@if ($mode == config('global.show') || isset($field['disabled'])) disabled @endif>
					...
					@case('password')
						<div class="col-sm-5">
							<input type="text" class="form-control" id="{{ $field['name'] }}"
								name="{{ $field['name'] }}" autocomplete="new-password"
								value="{{ isset($field['value']) ? old($field['name'], $field['value']) : old($field['name']) }}"
								@if ($mode == config('global.show') || isset($field['disabled'])) disabled @endif>
						</div>
						@if (isset($field['generate']))
							<div class="col-sm-3">
								<button type="button" name="get-password" id="get-password"
										...
										<option value="{{ $key }}"
											@if (isset($field['value']))
												@if (isset($field['multiple']) && $field['multiple'])
													@if (in_array($key, $field['value'])) selected @endif
												@elseif ($field['value'] == $key) selected @endif
											@endif
										>
											{{ $value }}</option>
									@endforeach
							...
							<div class="form-group btn-group col-lg-3 col-xs-12" role="group">
								...
								@foreach ($field['options'] as $key => $value)
									<input type="radio" class="btn-check" name="{{ $field['name'] }}" id="{{ $field['name'] . $item }}"
										autocomplete="off"
										@if ($mode == config('global.create')) @if ($loop->first) checked @endif @elseif
										($key == $field['value']) checked @endif
										@if ($mode == config('global.show')) disabled @endif
										value="{{ $key }}"
									>
