...
@section('steps')
	@php
		...
		$steps = [['title' => 'Работодатель', 'active' => false, 'context' => 'employer', 'link' => route('employers.index')], ['title' => 'Специальности<br/>Заявки от ОУ<br/>Практики работодателей', 'active' => true, 'context' => 'internship'], ['title' => $title, 'active' => false, 'context' => 'timetable']];
	@endphp
@endsection
...
@section('interior')
	<div class="block-header block-header-default">
		<div>
			<h3 class="block-title fw-semibold">Практики работодателей</h3>
			@if (isset(session('context')['chain']))
				Отображается единственная запись по цепочке из входящего сообщения
			@else
				<a href="{{ route('internships.create', ['sid' => session()->getId()]) }}" class="btn btn-primary mt-3 mb-3">Добавить
					практику</a>
			@endif
		</div>
	</div>
	<div class="block-content p-4">
		@if ($count > 0)
			<div class="table-responsive">
				<table class="table table-bordered table-hover text-nowrap" id="internships_table" style="width: 100%;">
					<thead>
						<tr>
