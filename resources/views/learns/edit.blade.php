...
@section('form.fields')
	@php
        ...
        } else $fields[] = ['name' => 'status', 'type' => 'hidden', 'value' => $learn->status];
        ...
        if ($learn->finish) $item['value'] = $learn->finish->format('d.m.Y');
        $fields[] = $item;
        if ($learn->status == \App\Models\ActiveStatus::ACTIVE->value) {
            ...
            $item = ['name' => 'school_id', 'title' => 'Учебное заведение', 'required' => false, 'type' => 'select', 'options' => $schools];
            if ($learn->school) $item['value'] = $learn->school->getKey();
            $fields[] = $item;
            $fields[] = ['name' => 'new_school', 'title' => 'Новое учебное заведение (нет в списке)', 'required' => false, 'type' => 'text', 'value' => $learn->new_school];
            ...
            if ($learn->specialty) $item['value'] = $learn->specialty->getKey();
            $fields[] = $item;
