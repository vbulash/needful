<div class="row mb-4">
	<label class=" col-sm-3 col-form-label" for="user_id">
		...
		@else
			Свяжите данную анкету образовательного учреждения с записью
			пользователя
		@endif
	...
	<div class="col-sm-5 col-form-label">
		<select name="user_id" id="user_id" class="form-control select2" @if ($mode == config('global.show')) disabled @endif>
			@hasrole(\App\Http\Controllers\Auth\RoleName::ADMIN->value)
				<option selected disabled>Выберите пользователя</option>
				@foreach ($users as $user)
					<option value="{{ $user['id'] }}"
						@if (isset($school)) @if ($school->user->getKey() == $user['id']) selected @endif
					@elseif(auth()->user()->getKey() == $user['id']) selected @endif>
						{{ $user['name'] }}
					</option>
				@endforeach
			@endhasrole
