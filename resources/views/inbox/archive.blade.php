...
@section('steps')
	@php
		$steps = [
			['title' => 'Входящие сообщения', 'active' => false, 'context' => null],
			['title' => 'Архив', 'active' => true, 'context' => null],
		];
	@endphp
@endsection
...
@section('blocks')
	...
	@if ($tasks->count() != 0)
		...
		<script>
			document.querySelectorAll('#card *').forEach(element => { element.setAttribute('disabled', 'disabled'); });
		</script>
	@endif
@endsection
...
@push('js_after')
	<script>
		...
		document.getElementById('card-restore').addEventListener('click', event => {
			$.ajax({
				method: 'GET',
