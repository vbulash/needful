...
EOB;
			    if ($trainee->hstatus == HistoryStatus::DESTROYED->value)
				    return $actions;
...
EOS,
	...
	public function index(): Factory|View|Application {
		...
		$students = $history->status == HistoryStatus::DESTROYED->value ? false : $this->getStudents($history);

		return view('trainees.index', compact('history', 'students', 'count'));
	}
	...
	private function invite(History $history, Student $student): void {
		...
		$student->notify(new New2AskedNotification($history, $student));
		event(new New2AskedTaskEvent($history, $student));
